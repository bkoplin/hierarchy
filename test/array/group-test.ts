...
import { map, mean, pipe, prop } from 'rambdax'
...
const groupByAge = group(
  data,
  'education_level',
  ['state_letter' as const, (x) => x.state[0]],
  'state'
)
...
describe('group function', () => {
  ...
  test('second level child dim is state', () => {
    expect(groupByAge.children[0].children[0].dim).toBe('state')
  })
  test('second level child has no children', () => {
  ...
  test('second level child depth is 2', () => {
  ...
  test('change value function, inline second level first child', () => {
    const c = groupByAge.children[0]

    c.setValueFunction(pipe(prop('records'), map(prop('crime_rate')), mean))
    c.setColor(undefined, 'allNodesAtDimValues')
    expect(c).toMatchFileSnapshot('./group-children.json')
  })
})

describe('ancestor tests', () => {
  test('ancestors tests', () => {
    const [level1Child] = groupByAge.leaves()
    const groupAncestor = groupByAge.children[0]
    const ancestors = level1Child.ancestors()
    const [firstAncestor, secondAncestor, thirdAncestor, root] = ancestors
    const [r, desc] = groupByAge.descendants()
    const dep = r.depth
  ...
  test("ancestorsAt depth of 1 of second level child has dim of 'education_level'", () => {
...
describe('leaf node tests', () => {
  ...
  test('first leaf node of root has depth of 2 and haschildren of false', () => {
  ...
  test("first leaf node of root has dim of 'state'", () => {
    expect(leaf?.dim).toBe('state')
  })
})
describe('links tests', () => {
  test('root generates a links json', () => {
    groupByAge.setColor('Spectral', 'allNodesAtDimIds')
...
describe('path tests', () => {
  test('root generates a path json', () => {
    const [first, last] = groupByAge.children
    const paths = last.leaves()[0].path(first.leaves()[0])
    const l = paths.length

    expect(paths.map((node) => node.depth)).toStrictEqual([2, 1, 0, 1, 2])
  })
})
describe('find tests', () => {
  test('find returns the correct node', () => {
    ...
    expect(found2.map((n) => [n.id, n.value, n.color, n.depth]))
...
describe('JSON test', () => {
  test('root generates json without parent or methods', () => {
