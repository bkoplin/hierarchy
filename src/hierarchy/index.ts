...
export function hierarchy<
  ...
  KeyFn2 extends StringKeyOf<T> | [StringKeyOf<T>, KeyFn<T>] | undefined,
  KeyFn3 extends StringKeyOf<T> | [StringKeyOf<T>, KeyFn<T>] | undefined,
  KeyFn4 extends StringKeyOf<T> | [StringKeyOf<T>, KeyFn<T>] | undefined,
  KeyFn5 extends StringKeyOf<T> | [StringKeyOf<T>, KeyFn<T>] | undefined,
  KeyFn6 extends StringKeyOf<T> | [StringKeyOf<T>, KeyFn<T>] | undefined
>(values: T[], key1: KeyFn1, key2?: KeyFn2, key3?: KeyFn3, key4?: KeyFn4, key5?: KeyFn5, key6?: KeyFn6) {
  type Height = KeyFn6 extends undefined ?
    KeyFn5 extends undefined ?
      KeyFn4 extends undefined ?
        KeyFn3 extends undefined ?
          KeyFn2 extends undefined ?
            KeyFn1 extends undefined ?
              1
              : 2
            : 3
          : 4
  ...
  const keyFuncs = [
    ...
    key5,
    key6,
  ].filter((k): k is KeyFn1 => typeof k !== 'undefined').map((k): KeyFn<T> => {
    if (typeof k === 'string')
      return (d: T) => d[k]
    else return k[1]
  }) as unknown as GroupArguments<T, Height>
  const dims = [
    key1,
    key2,
    key3,
    ...
    key5,
    key6,
  ].filter((k): k is KeyFn1 => typeof k !== 'undefined').map((k): StringKeyOf<T> => {
    if (typeof k === 'string')
      return k as StringKeyOf<T>
    else return k[0]
  }) as unknown as FixedLengthArray<StringKeyOf<T>, Height>
  const groupedData = group<T, Height>(
    values,
    ...keyFuncs as unknown as any
  ) as unknown as NestedMap<T, Height>
  const data = [
    undefined,
    groupedData,
  ] as const
  const root = new RootNode<T, Height>(
    data,
    dims,
    dims.length
  )
  const nodes: Array<RootNode<T, Height> | MiddleNode<T, Height, Height> | LeafNode<T, Height>> = [ root, ]
  let node = root
  let children = node.data ? Array.from(node.data[1]) : undefined
  ...
  while (node !== undefined) {
    if (typeof children !== 'undefined') {
      forEachRight(
        children,
        (child) => {
          const nodeChild = new MiddleNode<T, N.Sub<Height, 1>, N.Sub<Height, 1>>(
            child,
            dims,
            node.depth + 1,
            node.height - 1
    ...
    node = nodes.pop()
    children = node?.data?.[1] ? Array.from(node.data[1]) : undefined
  }
...
export abstract class AbstractNode<
  ...
  Dims extends Array<StringKeyOf<RecType>>
> {
  constructor(public data: Datum, public height: Height, public depth: Depth, public dims: Dims) {}
  #children?: Array<AbstractNode<RecType, Datum, N.Sub<Height, 1>, N.Add<Depth, 1>, Dims>>
  #parent?: AbstractNode<RecType, Datum, N.Add<Height, 1>, N.Sub<Depth, 1>, Dims>
   ...
   * @see {@link #dims}
   */
  #dim: StringKeyOf<RecType> | undefined = undefined
   ...
   * @memberof Node
   */
  dimPath: Array<StringKeyOf<RecType>> = []
  /**
  ...
  [Symbol.iterator] = function* (this: AbstractNode<
    RecType,
    Datum,
    Height,
    Depth,
  ...
  >) {
    ...
    let children: typeof this['children'] | undefined
  ...
  ancestors() {
    type Ancestor = {
      0: AbstractNode<RecType, Datum, N.Add<Height, 1>, N.Sub<Depth, 1>, Dims>
      1: AbstractNode<RecType, Datum, Height, Depth, Dims>
    }[N.IsZero<Depth>]
   ...
   */
  find(callback: (node: AbstractNode<RecType, Datum, Height, Depth, Dims>) => boolean) {
    for (const node of this) {
      if (callback(node))
        return node
    }
  ...
  hasChildren(): this is SetRequired<AbstractNode<RecType, Datum, Height, Depth, Dims>, 'children'> {
    return (this.children ?? []).length > 0
  }

  hasParent(): this is SetRequired<AbstractNode<RecType, Datum, Height, Depth, Dims>, 'parent'> {
    return !!this.parent
  }
  ...
  leaves<OnlyNodes extends boolean>(onlyNodes?: OnlyNodes) {
    type Leaf = {
      0: LeafNode<RecType, 0>
      1: MiddleNode<RecType, 1, 1>
  ...
  links(onlyNodes = false) {
    const links: Array<{
      source: AbstractNode<RecType, Datum, Height, Depth, Dims>
      target: AbstractNode<RecType, Datum, Height, Depth, Dims>
    }> = []
  ...
  setRecords() {
    return this.each((node) => {
      node.records = node.leaves().flatMap(leaf => leaf.data)
    })
  }
...
export class RootNode<RecType, Height extends number> extends AbstractNode<
  RecType,
  readonly [undefined, NestedMap<RecType, Height>],
  Height,
  0,
  Array<StringKeyOf<RecType>>
> {
  constructor(
    data: readonly [undefined, NestedMap<RecType, Height>],
    public dims: Array<StringKeyOf< RecType>>,
    height: Height
  ) {
    super(
      data,
...
export class MiddleNode<RecType, Height extends number = 1, Depth extends number = 1> extends AbstractNode<
  RecType,
  readonly [ValueOf<RecType>, NestedMap<RecType, Height>],
  Height,
  Depth,
  Array<StringKeyOf<RecType>>
> {
  constructor(
    data: readonly [ValueOf<RecType>, NestedMap<RecType, Height>],
    public dims: Array<StringKeyOf< RecType>>,
    height: Height,
    depth: Depth
  ) {
...
export class LeafNode<RecType, Depth extends number> extends AbstractNode<
  RecType,
  RecType[],
  0,
  Depth
...
export class Node<Height, Datum> {
  constructor(
    /**
  ...
  #children?:
  | undefined
  | Array<Node<Datum, Dims, I.Pos<I.Next<Depth>>, I.Next<Depth>>>
  ...
  #parent?: I.Pos<Depth> extends 0
    ? undefined
    : Node<Datum, Dims, I.Pos<I.Prev<Depth>>, I.Prev<Depth>>
  ...
  leaves<OnlyNodes extends boolean>(onlyNodes?: OnlyNodes) {
    type Leaf = {
      ...
      1: Node<
        ...
        ThisHeight,
        I.IterationOf<1>,
        I.IterationOf<N.Sub<ThisHeight, 1>>
      >
    }[A.Extends<OnlyNodes, true>]
    const leaves: Leaf[] = []
